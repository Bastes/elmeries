...
update action model =
  case action of
    Show ->
      ( { model | style = Style.animate |> Style.duration (0.5*second)
                                        |> Style.easing (\x -> x)
    ...
    Animate time ->
      ( { model | style = Style.tick time model.style }
        , Cmd.none
      )


view : Model -> Html Msg
view model = div [ onMouseEnter Show
                 , onMouseLeave Hide
                 , style
                     [ ( "position", "absolute" )
                     , ( "left", "0px" )
...
subscriptions model = AnimationFrame.times Animate
